...
QUnit.module('Тестируем функцию roman', function () {
	...
	QUnit.test('roman правильно обратывает невалидные числовые аргументы', function (assert) {
		const args = [
			Infinity,
			undefined,
			NaN,
		...
		args.forEach(arg => {
			assert.throws(() => roman(arg), TypeError('Incorrect argument'));
		});
	});

	QUnit.test('roman правильно обратывает невалидные символьные аргументы', function (assert) {
		const args = [
			'MLDPI',
			'5iljn',
			'IVX',
